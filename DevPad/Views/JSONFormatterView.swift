...
//  DevPad
//
//  Single-textbox JSON formatter (paste → tap "Format" → output replaces input).
//  Mirrors the jsonformatter.org workflow.
//
...
struct JSONFormatterView: View {
    ...
    private var controlBar: some View {
        HStack(spacing: 12) {
            ...
            Picker(settings.t("common.indent"), selection: $indent) {
                ...
                Text(settings.t("common.indent.tab")).tag(1)
            }
    ...
    private var editor: some View {
        ZStack(alignment: .topLeading) {
            ...
            TextEditor(text: $text)
                ...
                .padding(8)
            if text.isEmpty {
                Text(settings.t("json.placeholder"))
    ...
    private func errorBanner(_ message: String) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Image(systemName: "exclamationmark.triangle.fill")
                .foregroundStyle(.red)
            Text("\(settings.t("json.error.prefix")): \(message)")
                .font(.callout)
                .foregroundStyle(.red)
            Spacer()
        }
    ...
    private func format() {
        do {
            let useTab = (indent == 1)
            let formatted = try JSONFormatter.format(
                text,
                indent: useTab ? 2 : indent,
                sortKeys: false
            )
            text = useTab
    ...
    private func minify() {
        do {
            text = try JSONFormatter.minify(text)
            errorDetails = nil
        } catch {
            errorDetails = error.localizedDescription
        }
